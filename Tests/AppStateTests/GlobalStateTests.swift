...
final class GlobalStateTests: XCTestCase {
    ...
    var testState: GlobalState!
    
    override func setUp() {
        super.setUp()
        testState = GlobalState(initialState: initialState)
    }
    
    func testThatEmptyInitializerWorks() {
        self.measure() {
            let testState = GlobalState()
    ...
    func testThatInitializerWorksWithInitialState() {
        self.measure {
            let testState = GlobalState(initialState: initialState)
    ...
    func testThatStateEqualityWorks() {
        self.measure {
            ...
            let state2 = GlobalState(initialState: initialState)
            var state3 = GlobalState(initialState: initialState)
    ...
    func testThatStatesAreComparable() {
        self.measure {
            let state1 = testState ?? GlobalState(initialState: initialState)
            var state2 = GlobalState(initialState: initialState)
            var state3 = GlobalState(initialState: initialState)
    ...
    func testThatGlobalStateCopiesAreNotIndependent() {
        self.measure {
            var state1 = GlobalState(initialState: initialState)
            var state2 = state1
            XCTAssert(state1 == state2)
            XCTAssert(state1 === state2)
    ...
    static var allTests = [
        ...
        ("testThatGlobalStateCopiesAreNotIndependent", testThatGlobalStateCopiesAreNotIndependent),
    ]
}
